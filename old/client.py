    ...
    def equip(self, name):
        ...
        def send_equip_worker():
            try:
                ...
                print('failed to transmit equipment data to server')
        ...
        while True:
            while self.idling:
                ...
                if 'fight' in item:
                    ...
                    combat_result = recv_json(client)
                    if combat_result is None:
                        self.idling = False
                        break
                    player_hits = combat_result.get('player_hit', False)
                    player_damage = combat_result.get('player_damage', 0)
                    enemy_hits = combat_result.get('enemy_hit', False)
                    enemy_damage = combat_result.get('enemy_damage', 0)
                    print('fighting...')
                    print(1)
